...
Now we can see that the p-value of attitude is 4.12e-09 and that the model explains 18.56 % of the variation in points. We lost some percents but I still think the model with the variable that actually explains the dependent variable is better than a model that has useless non-statistically-significant explanatory variables in it. According to this model if the attitude increases by 1, the points increase by 3.5255. A similar relation can be seen also in the scatterplot drawn before. 

Finally I drew some diagnostic plots to see the goodness of the model I created.
...
In the first plot we can see residuals versus the fitted values. They should be vertically very randomly distributed on both sides of zero because in the ideal situation the residual that the model cannot explain will be random. So we are not missing some systematic thing going on that we didn't take into account. Luckily it seems that our model is successful in that aspect. The QQ-plot tells the same story - The more normally distributed the standardized residuals are, the better. In this case the points are quite beautifully on a straight line. The points in this and the previous plot are exactly the same but they are just plotted from a little bit different point of view. 
